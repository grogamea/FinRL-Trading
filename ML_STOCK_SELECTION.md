...
> **Personal note:** I initially confused `adj_close_q` with `trade_price` when first reading this — the table above saved me. The key insight is that tradedate is always ~2 months *after* quarter-end to account for the SEC filing window (10-Q deadline is 40 days for large accelerated filers). Don't shortcut this.
